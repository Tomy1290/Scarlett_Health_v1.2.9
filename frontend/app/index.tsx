...
function useThemeColors(theme: string) {
  ...
  const base = theme === "pink_pastel" ? { bg: "#fff0f5", card: "#ffe4ef", primary: "#d81b60", text: "#3a2f33", muted: "#8a6b75" }
    : theme === "pink_vibrant" ? { bg: "#1b0b12", card: "#2a0f1b", primary: "#ff2d87", text: "#ffffff", muted: "#e59ab8" }
...
function Onboarding({ onDone }: { onDone: () => void }) {
  ...
  const pages = [
    { t: language==='de'?'Tages-Navigation':'Daily navigation', d: language==='de'?'Mit Pfeilen durch Tage wischen, Heute-Button springt zurück.':'Use arrows to navigate days, Today jumps back.' },
    { t: language==='de'?'Einträge':'Entries', d: language==='de'?'Tabletten toggeln, Getränke zählen, Gewicht speichern, Sport markieren.':'Toggle pills, count drinks, save weight, mark sport.' },
    { t: language==='de'?'Ziele & Erinnerungen':'Goals & Reminders', d: language==='de'?'Ziel festlegen und Erinnerungen mit Benachrichtigungen aktivieren.':'Set a goal and enable reminders with notifications.' },
    { t: language==='de'?'Erfolge & Analyse':'Achievements & Analysis', d: language==='de'?'Erfolge sammeln, Verlauf analysieren und motiviert bleiben.':'Collect achievements, analyze progress, stay motivated.' },
  ];
  return (
    <View style={styles.onboardOverlay}>
      <View style={[styles.onboardCard, { backgroundColor: c.card }] }>
        <Text style={[styles.title, { color: c.text }]}>{pages[idx].t}</Text>
        <Text style={{ color: c.muted, marginTop: 8 }}>{pages[idx].d}</Text>
        <View style={{ flexDirection: 'row', gap: 6, marginTop: 12 }}>
          {pages.map((_, i) => (
            <View key={i} style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: i===idx? c.primary : c.muted, opacity: i===idx?1:0.5 }} />
          ))}
        </View>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 16 }}>
          <TouchableOpacity onPress={() => { onDone(); }} accessibilityLabel="Skip">
            <Text style={{ color: c.muted }}>{language==='de'?'Später':'Later'}</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => { if (idx < pages.length-1) { setIdx(idx+1); Haptics.selectionAsync(); } else { onDone(); } }} style={[styles.primaryBtn, { backgroundColor: c.primary }]}
            accessibilityLabel={language==='de'?'Weiter':'Next'}>
            <Text style={{ color: '#fff' }}>{idx < pages.length-1 ? (language==='de'?'Weiter':'Next') : (language==='de'?'Fertig':'Done')}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
...
export default function Home() {
  ...
  const {
    ...
    notificationIds, setNotificationId, hasSeededReminders, setHasSeededReminders,
  ...
  // Seed default reminders once
  useEffect(() => {
    (async () => {
      if (!hasSeededReminders) {
        await ensureNotificationPermissions();
        const defaults = [
          { type: 'pills_morning', time: '07:30' },
          { type: 'water', time: '12:00' },
          { type: 'sport', time: '18:00' },
          { type: 'gingerGarlicTea', time: '19:30' },
          { type: 'pills_evening', time: '20:30' },
        ];
        for (const def of defaults) {
          const r = { id: `seed_${def.type}`, type: def.type, time: def.time, enabled: true } as any;
          addReminder(r);
        }
        setHasSeededReminders(true);
      }
    })();
  }, [hasSeededReminders]);

  // Sync scheduled notifications with reminders
  useEffect(() => {
    (async () => {
      for (const r of reminders) {
        const key = r.id;
        const scheduled = notificationIds[key];
        if (r.enabled) {
          if (!scheduled) {
            const notifId = await scheduleDailyReminder(key, 'Erinnerung', `${r.type} – ${r.time}`, r.time);
            setNotificationId(key, notifId || undefined);
          }
        } else {
          if (scheduled) { await cancelNotification(scheduled); setNotificationId(key, undefined); }
        }
      }
    })();
  }, [reminders]);
  ...
  useEffect(() => { setWeightInput(d?.weight ? String(d.weight) : ""); }, [currentDate, d?.weight]);
  ...
  const header = (
    <View style={[styles.header, { backgroundColor: colors.card }]}>
      <TouchableOpacity onPress={goPrevDay} style={styles.iconBtn} accessibilityLabel="Prev day" hitSlop={{ top: 8, left: 8, right: 8, bottom: 8 }}>
      ...
      <View style={{ flexDirection: "row", alignItems: "center" }}>
        <TouchableOpacity onPress={goToday} style={styles.iconBtn} accessibilityLabel={t("today")} hitSlop={{ top: 8, left: 8, right: 8, bottom: 8 }}>
          <Ionicons name="calendar" size={20} color={colors.text} />
        </TouchableOpacity>
        <TouchableOpacity onPress={goNextDay} style={[styles.iconBtn, { opacity: currentDate === todayKey ? 0.3 : 1 }]} disabled={currentDate === todayKey} accessibilityLabel="Next day" hitSlop={{ top: 8, left: 8, right: 8, bottom: 8 }}>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.bg }] }>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalCard, { backgroundColor: colors.card }]}> 
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 12 }}>
              ...
              <TouchableOpacity style={[styles.primaryBtn, { backgroundColor: colors.primary }]} onPress={async () => { if (!/^\d{2}:\d{2}$/.test(newRemTime)) { Alert.alert('Fehler', 'Zeit bitte als HH:MM angeben'); return; } const r = { id: String(Date.now()), type: newRemType || 'custom', time: newRemTime, enabled: true } as any; addReminder(r); const notifId = await scheduleDailyReminder(r.id, 'Erinnerung', `${r.type} – ${r.time}`, r.time); useAppStore.getState().setNotificationId(r.id, notifId || undefined); setNewRemType(''); setNewRemTime(''); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); }}>
                <Ionicons name="add" size={18} color="#fff" />
              </TouchableOpacity>
            </View>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 8 }}>
              <TouchableOpacity onPress={async () => { const r = { id: `quick_sport_${Date.now()}`, type: 'sport', time: '18:00', enabled: true } as any; addReminder(r); const notifId = await scheduleDailyReminder(r.id, 'Erinnerung', `${r.type} – ${r.time}`, r.time); useAppStore.getState().setNotificationId(r.id, notifId || undefined); }} style={[styles.badge, { borderColor: colors.muted }]}>
                <Text style={{ color: colors.text }}>+ Sport 18:00</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={async () => { const r = { id: `quick_ginger_${Date.now()}`, type: 'gingerGarlicTea', time: '19:30', enabled: true } as any; addReminder(r); const notifId = await scheduleDailyReminder(r.id, 'Erinnerung', `${r.type} – ${r.time}`, r.time); useAppStore.getState().setNotificationId(r.id, notifId || undefined); }} style={[styles.badge, { borderColor: colors.muted }]}>
                <Text style={{ color: colors.text }}>+ Ingwer-Knoblauch 19:30</Text>
              </TouchableOpacity>
            </View>
            <View style={{ flexDirection: 'row', gap: 12, marginTop: 12 }}>
      ...
      {showOnboarding ? <Onboarding onDone={() => useAppStore.getState().setShowOnboarding(false)} /> : null}
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  onboardOverlay: { position: 'absolute', left: 0, right: 0, top: 0, bottom: 0, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.5)' },
  onboardCard: { width: '86%', borderRadius: 12, padding: 16 },
});