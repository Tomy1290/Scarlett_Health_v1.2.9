...
function perfectDaysCount(s: AchState) {
  return Object.values(s.days).filter(dayPerfect).length;
}
function longestPerfectStreak(s: AchState) {
  const keys = dayKeysSorted(s);
  let max = 0, cur = 0;
  for (let i = 0; i < keys.length; i++) {
    const k = keys[i];
    if (dayPerfect(s.days[k])) {
      if (i > 0 && isConsecutive(keys[i - 1], k) && dayPerfect(s.days[keys[i - 1]])) {
        cur += 1;
      } else {
        cur = 1;
      }
...
function complianceDays(s: AchState) {
  return Object.values(s.days).filter((d) => d.pills?.morning && d.pills?.evening).length;
}
...
function weightDelta(s: AchState) {
...
function weighedBeforeHourCount(s: AchState, hour = 8) {
  return count(Object.values(s.days), (d) => typeof d.weight === 'number' && typeof d.weightTime === 'number' && new Date(d.weightTime).getHours() < hour);
}
function trackedAfterHourCount(s: AchState, hour = 22) {
  return count(Object.values(s.days), (d) => typeof d.weightTime === 'number' && new Date(d.weightTime).getHours() >= hour);
}
...
const A: AchievementConfig[] = [
  ...
  // INTERMEDIATE 20-50
  { id: 'perfekte_woche_7', xp: 250, progress: (s) => Math.min(100, Math.round((perfectDaysCount(s)/7)*100)), title:(l)=> l==='de'?'Perfekte Woche':'Perfect week', description:(l)=> l==='de'?'7 Tage alle Ziele.':'7 perfect days.' },
  ...
  { id: 'erste_erfolge_2kg', xp: 400, progress: (s) => { const d = weightDelta(s); if (d <= -2) return 100; if (d >= 0) return 0; return Math.min(100, Math.round((Math.abs(d)/2)*100)); }, title:(l)=> l==='de'?'Erste Erfolge':'First success', description:(l)=> l==='de'?'2kg abgenommen.':'Lose 2kg.' },
  ...
  // ADVANCED 50-75
  { id: 'pillen_legende_100', xp: 1000, progress: (s) => Math.min(100, Math.round((complianceDays(s)/100)*100)), title:(l)=> l==='de'?'Pillen-Legende':'Pill legend', description:(l)=> l==='de'?'100 Tage alle Tabletten.':'100 days all pills.' },
  { id: 'perfekter_monat_30', xp: 750, progress: (s) => Math.min(100, Math.round((perfectDaysCount(s)/30)*100)), title:(l)=> l==='de'?'Perfekter Monat':'Perfect month', description:(l)=> l==='de'?'30 Tage alle Ziele.':'30 perfect days.' },
  { id: 'grosser_erfolg_5kg', xp: 800, progress: (s) => { const d = weightDelta(s); if (d <= -5) return 100; if (d >= 0) return 0; return Math.min(100, Math.round((Math.abs(d)/5)*100)); }, title:(l)=> l==='de'?'Großer Erfolg':'Big success', description:(l)=> l==='de'?'5kg abgenommen.':'Lose 5kg.' },

  // EXPERT 75-90
  { id: 'streak_master_50', xp: 1200, progress: (s) => Math.min(100, Math.round((longestPerfectStreak(s)/50)*100)), title:(l)=> l==='de'?'Streak-Master':'Streak master', description:(l)=> l==='de'?'50 Tage perfekt (Streak).':'50-day perfect streak.' },
  { id: 'jahres_champion_365', xp: 2500, progress: (s) => Math.min(100, Math.round((complianceDays(s)/365)*100)), title:(l)=> l==='de'?'Jahres-Champion':'Year champion', description:(l)=> l==='de'?'365 Tage Tabletten.':'365 days pills.' },
  { id: 'transformation_10kg', xp: 2000, progress: (s) => { const d = weightDelta(s); if (d <= -10) return 100; if (d >= 0) return 0; return Math.min(100, Math.round((Math.abs(d)/10)*100)); }, title:(l)=> l==='de'?'Transformation':'Transformation', description:(l)=> l==='de'?'10kg abgenommen.':'Lose 10kg.' },

  // LEGENDARY 90-100
  { id: 'diamant_status_100', xp: 3000, progress: (s) => Math.min(100, Math.round((perfectDaysCount(s)/100)*100)), title:(l)=> l==='de'?'Diamant-Status':'Diamond status', description:(l)=> l==='de'?'100 Tage perfekt.':'100 perfect days.' },
  ...
  { id: 'mega_transformation_20kg', xp: 5000, progress: (s) => { const d = weightDelta(s); if (d <= -20) return 100; if (d >= 0) return 0; return Math.min(100, Math.round((Math.abs(d)/20)*100)); }, title:(l)=> l==='de'?'Mega-Transformation':'Mega transformation', description:(l)=> l==='de'?'20kg abgenommen.':'Lose 20kg.' },
  { id: 'zen_meister_365', xp: 10000, progress: (s) => Math.min(100, Math.round((perfectDaysCount(s)/365)*100)), title:(l)=> l==='de'?'Zen-Meister':'Zen master', description:(l)=> l==='de'?'365 Tage perfekte Balance.':'365 days perfect balance.' },
